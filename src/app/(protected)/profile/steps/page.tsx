        ...
        async (keepSelectedUserDiseaseId?: string | null) => {
            try {
                ...
                if (keepSelectedUserDiseaseId) {
                    const found =
                        items.find((x) => x.userDiseaseId === keepSelectedUserDiseaseId) ?? null;
                    setSelected(found);
                }
            } catch (e: unknown) {
                const err = e as {
                    ...
                    <section className={styles.shell}>
                        <header className={styles.top}>
                            <div className={styles.topLeft}>
                                ...
                                <p className={styles.subtitle}>Работаем строго по user diseases / user steps</p>
                            </div>
                        ...
                        <div className={styles.layout}>
                            <section className={styles.left}>
                                <div className={styles.blockHead}>
                                    ...
                                    <p className={styles.blockHint}>Нажмите “Продолжить”, чтобы загрузить шаги</p>
                                </div>
                                ...
                                {diseasesLoading ? (
                                    ...
                                    <div className={styles.cards}>
                                        {diseases.map((d) => {
                                            const isSelected = selected?.userDiseaseId === d.userDiseaseId;
                                                    ...
                                                    <div className={styles.cardTop}>
                                                        <div className={styles.cardMeta}>
                                                            <div className={styles.cardTitleRow}>
                                                                <h3 className={styles.cardTitle}>{d.diseaseName}</h3>
                                                                <span className={styles.cardStatus}>{String(d.status || "—")}</span>
                                                            </div>
                                                    ...
                                                    <div className={styles.cardBottom}>
                                                        <div className={styles.cardDates}>
                                                            <div className={styles.dateLine}>
                                                                ...
                                                                <span className={styles.dateValue}>{fmtDate(d.startedAt)}</span>
                                                            </div>
                                                            <div className={styles.dateLine}>
                                                                <span className={styles.dateLabel}>Обновлено:</span>
                                                                <span className={styles.dateValue}>{fmtDate(d.updatedAt)}</span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        >
                                                            {stepsLoading && isSelected ? "Загрузка..." : "Продолжить"}
                                                        </button>
                                                    </div>
                                                </article>
                                            );
                            ...
                            <section className={styles.right}>
                                    ...
                                    {selected ? (
                                        <div className={styles.rightActions}>
                                            <span
                                                className={[
                                                    styles.diseaseBadge,
                                                    selectedResolved ? styles.diseaseBadgeDone : styles.diseaseBadgeActive,
                                                ].join(" ")}
                                            >
                                            ...
                                            <button
                                                ...
                                                title={selectedResolved ? "Уже завершено" : "Завершить болезнь"}
                                            >
                                ...
                                {!selected ? (
                                    <div className={styles.placeholder}>
                                        <div className={styles.placeholderTitle}>Нет выбранной болезни</div>
                                    ...
                                    <div className={styles.steps}>
                                        {steps.map((s, idx) => {
                                            const done = String(s.state || "").toLowerCase() === "completed";
                                                ...
                                                <div key={s.id} className={styles.step}>
                                                    ...
                                                    <div className={styles.stepMain}>
                                                        <div className={styles.stepTop}>
                                                            <div className={styles.stepTitle} title={`StepId: ${s.stepId} • UserStepId: ${s.id}`}>
                                                                ...
                                                                <span className={styles.stepKey}>UserStepId:</span>
                                                                <span className={styles.mono}>{String(s.id ?? "—")}</span>
                                                            </div>

                                                            <div className={styles.stepTopRight}>
                                                                <span className={[styles.badge, stateClass(s.state)].join(" ")}>
                                                                    {stateLabel(s.state)}
                                                                </span>
                                                                ...
                                                                <button
                                                                    ...
                                                                    disabled={done || completing || selectedResolved}
                                                                    title={
                                                                        selectedResolved
                                                                            ? "Болезнь завершена"
                                                                            : done
                                                                ...
                                                                >
                                                                    <CheckIcon className={styles.stepActionIcon} />
                                                                </button>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className={styles.stepGrid}>
                                                            <div className={styles.stepLine}>
                                                                <span className={styles.stepLabel}>Создан:</span>
                                                                <span className={styles.stepValue}>{fmtDate(s.createdAt)}</span>
                                                            </div>

                                                            <div className={styles.stepLine}>
                                                                <span className={styles.stepLabel}>Обновлён:</span>
                                                                <span className={styles.stepValue}>{fmtDate(s.updatedAt)}</span>
                                                            </div>

                                                            <div className={styles.stepLine}>
                                                                <span className={styles.stepLabel}>Завершён:</span>
                                                                <span className={styles.stepValue}>{fmtDate(s.completedAt)}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
